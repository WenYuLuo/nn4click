...
def generate_impulse_noise(num, number_class=2):
    if os.path.exists('./data_txt/noise_dirimpulse_data.txt'):
...
W_conv1 = weight_variable([1, 8, 1, 32])
...
def conv4(input_x, weight4, bias4):
    ...
    h_conv4 = conv2d(input_x, conv_weight4) + bias4
    return h_conv4
...
def conv5(input_x, weight5, bias5):
    ...
    h_conv5 = conv2d(input_x, conv_weight5) + bias5
    return h_conv5
...
def conv6(input_x, weight6, bias6):
    ...
    h_conv6 = conv2d(input_x, conv_weight6) + bias6
    return h_conv6
...
if __name__ == '__main__':
    ...
    # data1 = np.loadtxt('./data_txt/train_data.txt')
    # pl.plot(data1[0])
    # print(data1[0])
    click_label = np.loadtxt('shuffle_data_label.txt')
    label = np.tile(click_label[0:20000], (10, 1))
    ...
    flat_noise = high_pass_filter(flat_noise, cutoff=20000, fs=192000)
    ...
    impluse_noise = high_pass_filter(impluse_noise, cutoff=20000, fs=192000)
    ...
    label = np.vstack((label, flat_noise_label, impulse_noise_label))
    ...
    with tf.Session() as sess:
        ...
        for i in range(100):
            ...
            saver.save(sess, 'ckpt/allconv_cnn4click_mat_flat_impulse.ckpt', global_step=i)
            (train_data, train_label) = GetData.ShuffleData(train_data, train_label)
        print('training over.')

        # 获取第一个卷积层权重
        weight_conv1 = W_conv1.eval()
        reshaped_weight = weight_conv1.reshape((8, 32))
        np.savetxt('layer1_weight.txt', np.transpose(reshaped_weight))
