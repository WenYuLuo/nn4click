...
class wav_generator(object):
    ...
    def generate(self, click_num, verbose=False):
        ...
        while True:
            ...
            overlapped_length = crop_index[crop_index == 1].shape[0]
    ...
    # 内部生成信号函数
    def generate__(self, picked_dolphin, picked_noise, click_num, max_interval, noise_propotion=0.4, verbose=False):
        audio = np.zeros((1, 1))
        position = []
        i = 0
        while i < click_num:
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        hard_recog_label = np.zeros((1, 2))
        for i in range(300):
            data, label = generator.generate(click_num=18)
            ...
            idx = np.where(predict != truth)
            train_data_storage.push(data=data[idx[0]], label=label[idx[0]])
            pos_num, neg_num = count(label[idx[0]])
            if pos_num == 0:
                hard_recog_neg = np.vstack((hard_recog_neg, data[idx[0]]))
                hard_recog_label = np.vstack((hard_recog_label, label[idx[0]]))
            ...
            while True:
                ...
                step = step + 1
        np.savetxt('./data_txt/hard_recog.txt', hard_recog_neg[1:])
        np.savetxt('./data_txt/hard_label.txt', hard_recog_label[1:])
